...
s1 <-  runif(1,0,1); w1 <- runif(1,0,2); h1 <- runif(1,0,2); # GRE  
s2 <-  runif(1,0,1); w2 <- runif(1,0,1); h2 <- runif(1,0,1); # GPA                      
s3 <-  runif(1,0,1); w3 <- runif(1,0,1); h3 <- runif(1,0,1); # Research experience      
s4 <-  runif(1,0,1); w4 <- runif(1,0,1); h4 <- runif(1,0,1); # Ability to work in lab (as assessed by LORs)
...
s7 <-  0.0;          w7 <- runif(1,0,1); h7 <- runif(1,0,1); # Irrelevant factors used Harvard's admissions committee
s8 <-  runif(1,0,5); w8 <- 0.0;          h8 <- 0.0; # Chance effects
...
Corr12 <- runif(1,0,0.5) # Correlation between GRE and GPA
Corr34 <- runif(1,0,0.5) # Correlation between research experience and LORs
